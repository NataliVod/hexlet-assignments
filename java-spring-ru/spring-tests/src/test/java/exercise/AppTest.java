    ...
    void testUpdatesPerson() throws Exception {
        ...
        MockHttpServletResponse responsePost = mockMvc.perform(
                        patch("/people/{id}", existingUserId)
                                .contentType(MediaType.APPLICATION_JSON)
                                .content("{\"firstName\": \"Cat\", \"lastName\": \"Catson\", \"email\": \"catmail@gmail.com\"}")
                )
                .andReturn()
                .getResponse();
        ...
        assertThat(response.getStatus()).isEqualTo(200);
        assertThat(response.getContentType()).isEqualTo(MediaType.APPLICATION_JSON.toString());
        assertThat(response.getContentAsString()).contains("Cat", "Catson","catmail@gmail.com");
        assertThat(response.getContentAsString()).doesNotContain("John", "Smith", "john@gmail.com");
    }
